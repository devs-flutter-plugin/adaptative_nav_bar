...
const List<AdaptiveNavDestination> _fiveDestinations = <AdaptiveNavDestination>[
...
void main() {
  ...
  testWidgets('google style gives the selected tab real extra row width', (
    ...
    final Finder homeSlot = find.byKey(const ValueKey<String>('google-slot-0'));
    final Finder searchSlot = find.byKey(
      const ValueKey<String>('google-slot-1'),
    );
    expect(
      tester.getSize(homeSlot).width,
      greaterThan(tester.getSize(searchSlot).width),
    );
    ...
    expect(
      tester.getSize(searchSlot).width,
      greaterThan(tester.getSize(homeSlot).width),
    );
  ...
  testWidgets(
    'persistent style uses the reference 2 to 1 selected slot ratio',
  ...
  testWidgets(
    'notch selected button is centered exactly on its destination slot',
    (WidgetTester tester) async {
      await _setSurface(tester, const Size(390, 800));
      ...
      final double homeY = tester
          .getCenter(find.byIcon(Icons.home_outlined))
          .dy;
  ...
  testWidgets(
    'centerRaised keeps the primary destination fixed at screen center',
    (WidgetTester tester) async {
      await _setSurface(tester, const Size(390, 800));
      int selected = 0;
